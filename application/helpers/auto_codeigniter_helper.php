<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	if(!function_exists('aci_ui_a'))
	{
		...
		function aci_ui_a($folder,$controller,$method,$args,$attr,$html,$is_return=false)
		{
			...
			if($CI->current_role_priv_arr){
				...
				if(!$found)
				foreach($CI->current_role_priv_arr as $k=>$v){
					if(strtolower($v['method'])==strtolower($method)&&strtolower($v['controller'])==strtolower($controller)&&strtolower($v['folder'])==strtolower($folder)){
						$found=true;
						break;
					}
				}
	...
	if(!function_exists('http_get_data'))
	{
		function http_get_data($url){
			...
			curl_setopt($curl, CURLOPT_REFERER, 'https://www.internationalparceltracking.com');
